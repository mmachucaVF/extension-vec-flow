(() => {
  ...
  function buildPanel() {
    ...
    panel.innerHTML = `
      ...
      <div id="fm-main">
        ...
        <div id="fm-detail">
          <div class="fm-no-sel"><div class="fm-no-sel-icon">â·</div><div class="fm-no-sel-text">SelecciÃ³nÃ¡ un flow<br>para ver el detalle</div></div>
        </div>
      </div>
  ...
  async function loadFlows() {
    ...
    try {
      ...
      setMsg('â', 'Cargando flows y estados...');
      ...
      renderList();
      setMsg('â', `Listo: ${total} flows cargados`);
  ...
  async function fetchStateTotals() {
    ...
    for (let page = 1; page <= 50; page++) {
      ...
      setMsg('â', `Cargando flows... ${flows.length}`);
  ...
  async function loadStateFromDOM(state) {
    ...
    for (let page = 1; page <= 50; page++) {
      setMsg('â', `Cargando ${label}... p.${page}`);
  ...
  async function loadAllFlows() {
    ...
    for (let page = 1; page <= 50; page++) {
      try {
        ...
        setMsg('â', `Cargando flows... ${flows.length}/${total}`);
        if (pf.length === 0 || total === 0 || to >= total) break;
      } catch(e) { break; }
    }
    return flows;
  ...
  function toggleGroupByError() {
    ...
    if (groupByError) {
      ...
      if (allLoaded) {
        renderList(); // ya estÃ¡n en cache, mostrar directo
      } else {
    ...
    for(const g of sorted){
      ...
      html+=`<div><div class="fm-error-group-hdr" data-action="toggle-err-group" data-group-key="${esc(g.key)}" style="background:${bg}">
        ...
        <div class="fm-error-group-meta" onclick="event.stopPropagation()">
          ...
          <button class="fm-chip" style="font-size:9px;padding:2px 7px" data-action="sel-group" data-group-key="${esc(g.key)}">${allSel?'âï¸':'â'} sel.</button>
          <button class="fm-chip" style="font-size:9px;padding:2px 7px;background:rgba(108,99,255,.15);border-color:rgba(108,99,255,.35);color:#8c85ff" data-action="ticket-group" data-group-key="${esc(g.key)}">ð« ticket</button>
        </div>
  ...
  function clearDetail(){const p=document.getElementById('fm-detail');p.innerHTML=`<div class="fm-no-sel"><div class="fm-no-sel-icon">â·</div><div class="fm-no-sel-text">SelecciÃ³nÃ¡ un flow<br>para ver el detalle</div></div>`;}
  ...
  function selectGroup(k) {
    ...
    const allSel = groupFlows.every(f => selectedIds.has(f.id));
    if (allSel) {
